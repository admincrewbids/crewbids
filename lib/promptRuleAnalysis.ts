export type ParsedPreferenceFilterLike = {
  field: string;
  operator: string;
...
export type ParsedPreferenceSortLike = {
  field: string;
  direction: "asc" | "desc";
  strength: string;
  weight?: number;
...
export type ParsedPreferencePriorityGroupLike = {
  rank: number;
  strength: string;
  conditions: {
    field: string;
...
export type ScopedPreferenceLike = {
  terminal: string;
  normalized_terminal: string;
  priority_rank: number;
  sort_preferences: ParsedPreferenceSortLike[];
  filters: ParsedPreferenceFilterLike[];
  required_days_off: string[];
  requires_weekends_off: boolean;
};

export type ParsedPreferencesLike = {
  filters: ParsedPreferenceFilterLike[];
  priority_groups: ParsedPreferencePriorityGroupLike[];
  sort_preferences: ParsedPreferenceSortLike[];
  tradeoffs: {
    type: string;
    value?: string;
  ...
  scoped_preferences?: ScopedPreferenceLike[];
};

export type CanonicalRuleCategory =
  | "priority"
  | "hard_filter"
  | "preference"
  | "tradeoff"
...
export type CanonicalRuleScope = "global" | "terminal";

export type CanonicalPreferenceRule = {
  category: CanonicalRuleCategory;
...
export type PromptInterpretationIssue = {
...
export type PromptRuleAnalysis = {
  rules: CanonicalPreferenceRule[];
  issues: PromptInterpretationIssue[];
};
...
function humanizeOperator(operator: string) {
  switch (operator) {
    case "in":
      return "allows";
    case "not_in":
...
function formatFilterRuleLabel(
  ...
  if (
    ...
    return `Only show crews from ${formatNaturalList(
      filter.value.map((value) => formatTerminalDisplayName(String(value)))
    )}`;
  }
  ...
  if (
    ...
    return `Hide crews from ${formatNaturalList(
      filter.value.map((value) => formatTerminalDisplayName(String(value)))
    )}`;
  }
  ...
  return `${humanizeFieldName(filter.field)} ${humanizeOperator(
    filter.operator
  )} ${formatValue(filter.value)}`;
}

function formatSortRuleLabel(sort: ParsedPreferenceSortLike) {
  ...
  return `Sort by ${humanizeFieldName(sort.field)} ${
    sort.direction === "asc" ? "(low to high)" : "(high to low)"
  }`;
}

function pushScopeIssue(
  issues: PromptInterpretationIssue[],
  issue: PromptInterpretationIssue
) {
...
function analyzeScopeForIssues(
  filters: ParsedPreferenceFilterLike[],
  sorts: ParsedPreferenceSortLike[],
  requiredDaysOff: string[],
  requiresWeekendsOff: boolean,
  scope: CanonicalRuleScope,
  terminal: string | undefined,
  issues: PromptInterpretationIssue[]
) {
  const minStart = filters.find(
    (filter) => filter.field === "on_duty" && filter.operator === ">="
  );
  const maxStart = filters.find(
    (filter) => filter.field === "on_duty" && filter.operator === "<="
  );
  const minFinish = filters.find(
    (filter) => filter.field === "off_duty" && filter.operator === ">="
  );
  const maxFinish = filters.find(
    (filter) => filter.field === "off_duty" && filter.operator === "<="
  );
  const weekdaysOnly = filters.some(
    (filter) =>
      filter.field === "weekend_days_off" &&
      filter.operator === "=" &&
      filter.value === false
  );
  const weekendsOnly = requiresWeekendsOff || filters.some(
    (filter) =>
      filter.field === "weekends_off_hard" &&
      filter.operator === "=" &&
      filter.value === true
  );
  const onlyThreeDayOff = filters.some(
    (filter) =>
      filter.field === "include_only_three_day_off_jobs" &&
      filter.operator === "=" &&
      filter.value === true
  );
  const noThreeDayOff = filters.some(
    (filter) =>
      filter.field === "exclude_three_day_off_jobs" &&
      filter.operator === "=" &&
      filter.value === true
  );
  const onlySpareboard = filters.some(
    (filter) =>
      filter.field === "include_only_spareboard_crews" &&
      filter.operator === "=" &&
      filter.value === true
  );
  const noSpareboard = filters.some(
    (filter) =>
      filter.field === "exclude_spareboard_crews" &&
      filter.operator === "=" &&
      filter.value === true
  ...
  const sortDirectionsByField = new Map<string, Set<string>>();
...
export function analyzeParsedPreferences(
  parsed: ParsedPreferencesLike,
  formatTerminalDisplayName: (terminal: string | undefined) => string
): PromptRuleAnalysis {
  const rules: CanonicalPreferenceRule[] = [];
  const issues: PromptInterpretationIssue[] = [];
  ...
  for (const group of parsed.priority_groups ?? []) {
    ...
    if (!terminalCondition) continue;

    rules.push({
      category: "priority",
      scope: "terminal",
      terminal: formatTerminalDisplayName(String(terminalCondition.value)),
      label: `Priority ${group.rank}: ${formatTerminalDisplayName(
        String(terminalCondition.value)
      )}`,
  ...
  for (const scope of parsed.scoped_preferences ?? []) {
    const terminalLabel = formatTerminalDisplayName(
