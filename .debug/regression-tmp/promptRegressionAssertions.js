...
exports.evaluatePromptRegressionAssertions = evaluatePromptRegressionAssertions;
function normalizeTerminalLabel(value) {
    return String(value ?? "").trim().toLowerCase();
}
function isRecord(value) {
    return typeof value === "object" && value !== null;
}
function isParsedPreferenceFilterLike(value) {
...
function isParsedPreferenceSortLike(value) {
    return (isRecord(value) &&
        typeof value.field === "string" &&
        (value.direction === "asc" || value.direction === "desc") &&
        typeof value.strength === "string");
}
function isParsedPreferencesLike(value) {
    return (isRecord(value) &&
...
function toRankedCrewLike(crew) {
    return crew;
}
function valuesEqual(left, right) {
    return JSON.stringify(left) === JSON.stringify(right);
}
function matchesFilterExpectation(filter, expected) {
    if (filter.field !== expected.field)
        return false;
    if (expected.operator != null && filter.operator !== expected.operator)
        return false;
    if (expected.strength != null && filter.strength !== expected.strength)
        return false;
...
function matchesSortExpectation(sort, expected) {
    if (sort.field !== expected.field)
        return false;
    if (sort.direction !== expected.direction)
        return false;
    if (expected.strength != null && sort.strength !== expected.strength)
        return false;
    return true;
}
function getScopedPreference(parsed, terminal) {
    const normalized = normalizeTerminalLabel(terminal);
    return (parsed.scoped_preferences ?? []).find((scope) => normalizeTerminalLabel(scope.normalized_terminal) === normalized ||
...
function getDisplayedDayTimeRange(day) {
    const jobStart = typeof day?.job_detail?.on_duty === "string" ? day.job_detail.on_duty : null;
    const jobFinish = typeof day?.job_detail?.off_duty === "string" ? day.job_detail.off_duty : null;
...
function isOvernightDisplayedDay(day) {
...
function getDayFinishMinutes(day) {
    const { onDuty, offDuty } = getDisplayedDayTimeRange(day);
    const startMinutes = hhmmToMinutes(onDuty);
...
function getRepresentativeWorkedDay(crew) {
    return getWorkedDays(crew)[0] ?? null;
}
function getSortableFieldValue(crew, field) {
    const representativeDay = getRepresentativeWorkedDay(crew);
    const representativeDetail = representativeDay?.job_detail ??
        crew.job_details?.[0] ??
        null;
    if (field === "on_duty") {
...
function evaluateFinishFilterForDay(day, filter) {
    ...
    const isEarlyMorningCutoff = rawFilterMinutes != null && rawFilterMinutes < 12 * 60;
...
function getWorkedDays(crew) {
    return (crew.daily ?? []).filter((day) => !day?.is_day_off);
}
function hasSplitTimeValue(value) {
    if (typeof value !== "string")
        return false;
    const cleaned = value.trim();
    if (!cleaned || cleaned === "-" || cleaned === "00:00")
        return false;
    return cleaned !== "0:00";
}
function hasShuttleBusValue(value) {
    if (typeof value === "boolean")
        return value;
    if (typeof value === "string") {
        return (/shuttle\s*bus/i.test(value) ||
            /^shuttle\b/im.test(value) ||
            /\bshuttle\b.*\b\d{1,2}:\d{2}\b/i.test(value));
    }
    return false;
}
function crewHasShuttleBus(crew) {
    for (const day of crew.daily ?? []) {
        if (day?.is_day_off)
            continue;
        if (hasShuttleBusValue(day?.job_detail?.has_shuttle_bus) ||
            hasShuttleBusValue(day?.job_detail?.raw_text)) {
            return true;
        }
    }
    for (const detail of crew.job_details ?? []) {
        if (hasShuttleBusValue(detail?.has_shuttle_bus) ||
...
function evaluateHardFilterOnCrew(crew, filter) {
    ...
    if (filter.field === "terminal") {
        ...
        const matchesSpecialValue = normalizedValues.some((value) => {
            if (value === "standby")
                return isStandbyCrew(crew);
            if (value === "spareboard")
                return isSpareboardCrew(crew);
...
function collectVisibleContradictionFailures(result, parsed) {
    const failures = [];
    for (const crew of result.ranked) {
        const inspectableCrew = toRankedCrewLike(crew);
        for (const filter of parsed.filters ?? []) {
            if (filter.strength !== "hard")
                continue;
...
function evaluateConditionalTerminalFallbackAssertion(result, primaryTerminal, requiredFilters, fallbackTerminal) {
    ...
    const primaryCrews = rankedCrews.filter((crew) => normalizeTerminalLabel(crew.terminal) === normalizedPrimaryTerminal);
    const fallbackCrews = rankedCrews.filter((crew) => normalizeTerminalLabel(crew.terminal) === normalizedFallbackTerminal);
    const qualifyingPrimaryCrews = primaryCrews.filter((crew) => requiredFilters.every((expectedFilter) => {
...
function evaluateScopedRankOrderRespectsSortAssertion(result, terminal, field, direction, mode, requireAtLeastComparablePairs) {
    ...
    for (let index = 0; index < scopedCrews.length - 1; index += 1) {
        ...
        if (currentValue === nextValue)
            continue;
        comparablePairs += 1;
        const respectsOrder = direction === "asc"
...
function evaluateScopedTerminalSuppressesGlobalSortAssertion(result, terminal, suppressedGlobalSort, enforcedScopedSorts, mode, requireAtLeastComparablePairs) {
    ...
    for (let index = 0; index < scopedCrews.length - 1; index += 1) {
        const currentCrew = scopedCrews[index];
        const nextCrew = scopedCrews[index + 1];
        const currentSuppressedValue = getSortableFieldValue(currentCrew, suppressedGlobalSort.field);
        const nextSuppressedValue = getSortableFieldValue(nextCrew, suppressedGlobalSort.field);
        if (currentSuppressedValue == null || nextSuppressedValue == null)
            continue;
        if (currentSuppressedValue === nextSuppressedValue)
            continue;
        const respectsSuppressedGlobalSort = suppressedGlobalSort.direction === "asc"
        ...
        for (const scopedSort of enforcedScopedSorts) {
            ...
            if (currentScopedValue == null || nextScopedValue == null)
                continue;
            if (currentScopedValue === nextScopedValue)
                continue;
            hasComparableScopedSort = true;
            const respectsScopedSort = scopedSort.direction === "asc"
...
function evaluateAssertion(assertion, result, summary, parsed) {
    ...
    if (assertion.type === "parsed_priority_terminal_absent") {
        ...
        const found = (parsed.priority_groups ?? []).some((group) => group.conditions.some((condition) => condition.field === "terminal" &&
    ...
    if (assertion.type === "parsed_global_filter_present") {
        const found = (parsed.filters ?? []).some((filter) => matchesFilterExpectation(filter, assertion.value));
        return found
            ? null
            : {
                type: assertion.type,
    ...
    if (assertion.type === "parsed_scoped_filter_present") {
        ...
        const found = (scoped?.filters ?? []).some((filter) => matchesFilterExpectation(filter, assertion.value.filter));
        return found
            ? null
            : {
                type: assertion.type,
    ...
    if (assertion.type === "parsed_global_sort_present") {
        const found = (parsed.sort_preferences ?? []).some((sort) => matchesSortExpectation(sort, assertion.value));
        return found
            ? null
            : {
                type: assertion.type,
    ...
    if (assertion.type === "parsed_scoped_sort_present") {
        const scoped = getScopedPreference(parsed, assertion.value.terminal);
        const found = (scoped?.sort_preferences ?? []).some((sort) => matchesSortExpectation(sort, assertion.value.sort));
        return found
            ? null
            : {
                type: assertion.type,
    ...
    if (assertion.type === "crew_ranked") {
        const match = result.ranked.find((crew) => String(crew.id).trim() === assertion.value.crewId.trim());
        if (!match) {
            return {
                type: assertion.type,
    ...
    if (assertion.type === "crew_excluded") {
        const match = result.excluded.find((crew) => String(crew.id).trim() === assertion.value.crewId.trim());
        if (!match) {
            return {
                type: assertion.type,
        ...
        if (assertion.value.reasonIncludes &&
    ...
    if (assertion.type === "tradeoff_present") {
        const found = (parsed.tradeoffs ?? []).some((tradeoff) => tradeoff.type === assertion.value.type &&
    ...
    if (assertion.type === "avoid_is_not_hard") {
        ...
        const disallowedMatches = (parsed.filters ?? []).filter((filter) => {
            ...
            if (normalizedTerminal &&
                ...
                return filter.value.some((value) => normalizeTerminalLabel(String(value)) === normalizedTerminal);
            }
            return false;
        });
    ...
    if (assertion.type === "conditional_terminal_fallback") {
        return evaluateConditionalTerminalFallbackAssertion(result, assertion.value.primary.terminal, assertion.value.primary.requires, assertion.value.fallback.terminal);
    }
    if (assertion.type === "scoped_rank_order_respects_sort") {
        return evaluateScopedRankOrderRespectsSortAssertion(result, assertion.value.terminal, assertion.value.field, assertion.value.direction, assertion.value.mode ?? "pairwise_consecutive_distinct", assertion.value.requireAtLeastComparablePairs ?? 1);
    }
    if (assertion.type === "scoped_terminal_suppresses_global_sort") {
        return evaluateScopedTerminalSuppressesGlobalSortAssertion(result, assertion.value.terminal, assertion.value.suppressedGlobalSort, assertion.value.enforcedScopedSorts, assertion.value.mode ?? "consecutive_pair_evidence", assertion.value.requireAtLeastComparablePairs ?? 1);
    }
    if (assertion.type === "interpretation_issue_absent") {
        const matchingIssue = (result.interpretationIssues ?? []).find((issue) => {
            if (assertion.value.code && issue.code !== assertion.value.code)
                return false;
            if (assertion.value.scope && issue.scope !== assertion.value.scope)
                return false;
            if (assertion.value.messageIncludes &&
...
function evaluatePromptRegressionAssertions(regressionCase, result, summary) {
    if (!isParsedPreferencesLike(result.parsedPreferences)) {
        return [
            {
                type: "mechanical_verification",
                message: "Regression assertions could not run because parsedPreferences did not match the inspectable debug shape.",
            },
        ];
    }
